...
class BasePage {
    ...
    notificationToast = (timeoutValue: number = 30000) => cy.get('.notification-toast', {timeout: timeoutValue});
    footer = {
    ...
    public getDataSliceFromTable(tableBodyElement: Chainable<JQuery<HTMLElement>>, flattenData: boolean, x: number, y: number, column: number, row?: number) {
    ...
    public successNotificationPopped(successMessage?: string, waitForPopOut: boolean = false) {
        ...
        if (successMessage) {
            this.successNotification().parent().siblings(`div:contains("${successMessage}")`).should('be.visible');
        }
        if (waitForPopOut) {
    ...
    public errorNotificationPopped(errorMessage?: string, waitForPopOut: boolean = false) {
        ...
        if (errorMessage) {
            this.errorNotification().parent().siblings(`div:contains("${errorMessage}")`).should('be.visible');
        }
        if (waitForPopOut) {
