...
describe('Country Information API Test Suite', () => {
    it('Get all countries', {tags: ['@get-countries', '@smoke']}, () => {
        cy.sendApiRequestDef(apiEndpoints.getAllCountries, HttpMethod.GET, 'null', StatusCodes.OK).then((response) => {
    ...
    it('Get country by name', {tags: ['@get-countries', '@smoke']}, () => {
        ...
        cy.sendApiRequest(apiEndpoints.getCountryByName(countryName), HttpMethod.GET, 'null', StatusCodes.OK).then((response) => {
            const country = response[0] as CountryResponses.Country;
            expect(country).to.have.property('name');
    ...
    it('Get countries by language', {tags: ['@get-countries', '@smoke']}, () => {
        const language = 'eng';
        cy.sendApiRequest(apiEndpoints.getCountryByLanguage(language), HttpMethod.GET, 'null', StatusCodes.OK).then((response) => {
            expect(response).to.be.an('array');
            const country = response[0] as CountryResponses.Country;
            expect(country).to.have.property('name');
            expect(country).to.have.property('languages');
            ...
            if (Array.isArray(country.languages)) {
                const hasLanguage = country.languages.some((lang: CountryResponses.Language) =>
